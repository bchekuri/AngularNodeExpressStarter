 ...
 * @param app 
 */
function initilizeViewRoutes(app) {
    app.use(/^((?!(api)).)*/, (req, res) => {
        res.locals.title = config.has('title') ? config.get('title') : 'starter';
        res.status(200);
        res.render(config.has('server.indexView') ? config.get('server.indexView') : 'index.view.ejs');
    });
}
...
function errorHandler(app) {
    app.use(function(err, req, res, next) {
        ...
        res.render(config.has('server.errorView') ? config.get('server.errorView') : 'error.view.ejs');
    });
}
